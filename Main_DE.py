...
if only_bp:
    bp = ann.BP_ANN(h_layer_num, l_rate, momentum, n_epoch, batch_size, valid_rate, opt="adam", do=True)
    fit = bp.train_one(None, is_bp=only_bp)
    print("Global loss at the end of the training: ", fit)
else:
    print("---- DIFFERENTIAL EVOLUTION IS ON ----")
    sleep(2)
    bp = ann.BP_ANN(h_layer_num, l_rate, momentum, n_epoch, batch_size, valid_rate, opt="nesterov", do=False)
    ...
    result = differential_evolution(BP_fit_fun, bounds, maxiter=1000, popsize=20, disp=True, tol=0.000001)
