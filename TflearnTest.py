...
net = tflearn.fully_connected(net, 5, activation='sigmoid', bias=False)
...
ran_vector = np.random.rand(sum(shape_len_list))
...
for act_v, value in zip(v_l, assign_vector):
    tflearn.variables.set_value(act_v, value, session=None)
    print("egyszer")
